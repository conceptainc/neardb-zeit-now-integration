...
module.exports = withUiHook(async({ payload, zeitClient }) => {
  ...
  const baseReturn = htm`
    <Page>
      <Container>
        <P>Please, create an AWS S3 account.</P>
        <Input name="secret" label="Secret" value="${metadata.secret}"/>
        <Input name="bucketName" label="Bucket Name" value="${metadata.bucketName}"/>
        <Input name="region" label="Region" value="${metadata.region}"/>
        <Input name="apiKey" label="API Key" value="${metadata.apiKey}"/>
        <Input name="cachingOptions" label="Caching Options" value="${metadata.cachingOptions}"/>
      </Container>
      <Container>
